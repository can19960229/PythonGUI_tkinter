...
class MainForm: 							# 主窗体
    ...
    def show_canvas(self):
        ...
        self.canvas_write = tkinter.Canvas(self.root, height=900, width=750)  # 创建绘图板
        ...
        self.canvas_write.pack(side='right', expand='yes', fill='both',padx=5, pady=5)  # 画布显示
    ...
    def scale(self):
        self.scale = tkinter.Scale(self.canvas_write, label="拖动调整文字大小",
                                   from_=1,  # 最小值
                                   to=100,  # 最大值
                                   orient=tkinter.HORIZONTAL,  # 水平方向拖动
                                   length=800,  # 滑块长度
                                   showvalue=True,  # 显示当前值
                                   tickinterval=10,  # 选项间隔
                                   resolution=True)  # 整数显示
        self.scale.bind("<B1-Motion>", self.change_font_handle)  # 绑定拖动事件
        self.scale.pack(side='top', expand='no', anchor='nw', padx=5, pady=5)  # 组件显示

    def change_font_handle(self, event):  # 操作处理
        self.label_text.configure(font=("微软雅黑", self.scale.get()))  # 修改字体
    ...
    def createwindow(self):
        ...
        # button
        self.button = tkinter.Button(self.canvas_write, text="set label to text", width=15, height=2,
    ...
    def src_listbox(self):  # 定义列表
        self.language_label = tkinter.Label(self.canvas_write, text="请选择你需要的器件规格：",
                                            bg="#223011", font=("微软雅黑", 9), fg="#ffffff", justify="left").pack(side="left",anchor='nw',padx=5, pady=5)  # 组件显示
    ...
    def dest_listbox(self):  # 定义列表
        self.choose_label = tkinter.Label(self.canvas_write, text="请画出你的电路：", bg="#223011",
                                          font=("微软雅黑", 9), fg="#ffffff", justify="left").pack(side="left",anchor='nw',padx=5, pady=5)  # 组件显示
        # 列表项，多选模式
        self.choose_listbox = tkinter.Listbox(self.canvas_write,
                                              selectmode="multiple").pack(side="left",anchor='nw',padx=5, pady=5)  # 组件显示
    #  列表修改
