...
data = open("F:\\PYECourse\\2\\data\\test_data.txt", "r")
map_record = {}
for i in data:
        ...
        for each_node in sorted(list(que)):
            if each_node in map_record:
                for add_node in sorted(map_record[each_node]):
                    if add_node == key:
                        if i > 1:
                            for each_position in position[each_node]:
                                sum_len = sum_len + 1
                                result[i].append(path[each_position])
                    elif add_node > key:
                        if add_node not in new_node:
                            new_node.add(add_node)
                            new_position[add_node] = []
                            for each_position in position[each_node]:
                                if add_node not in path[each_position]:
                                    new_path[cnt] = path[each_position] + [add_node]
...
file = open("F:\\PYECourse\\2\\data\\result_my.txt", "w")
